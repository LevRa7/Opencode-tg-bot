...
export async function getBotServiceStatus(): Promise<BotServiceStatus> {
  ...
  if (!service) {
    return {
      ...
      service: null,
      cleanupReason,
      message: "Bot service is stopped.",
    };
  }
  ...
  if (!isProcessAlive(service.pid)) {
    ...
    return {
      ...
      service: null,
      cleanupReason: "stale",
      message: "Bot service is stopped (stale).",
    };
  }
...
export async function stopBotDaemon(
  ...
  if (currentStatus.status !== "running" || !currentStatus.service) {
    return {
      ...
      cleanupReason: currentStatus.cleanupReason,
      alreadyStopped: true,
      message: "Bot service is already stopped.",
    };
  }
  ...
  try {
    ...
    return {
      ...
      message: `Failed to stop background bot process PID=${pid}.`,
    };
    }
    ...
    return {
      ...
      service: currentStatus.service,
      cleanupReason: currentStatus.cleanupReason,
      message: "Bot service stopped successfully.",
    };
  } catch (error) {
    return {
      success: false,
...
export const serviceManager = {
  startService: startBotDaemon,
  stopService: stopBotDaemon,
  getServiceStatus: getBotServiceStatus,
};
