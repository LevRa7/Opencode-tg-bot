...
vi.mock("grammy", () => {
  class FakeBot {
    ...
    command(_command: string, _handler: (...args: any[]) => any): this {
      commandHandlers.set(_command, _handler);
      return this;
    }
...
const EXPECTED_COMMANDS = [
  "start", "help", "status", "stream", "restart", "tts", "terminal",
  "opencode_start", "opencode_stop", "projects", "sessions", "model",
  "variant", "compact", "settings", "new", "abort", "detach",
  "share", "unshare", "fork", "revert", "del", "memory", "server",
  "connect", "task", "tasklist", "rename", "commands", "worktree",
  "open", "ls", "skills", "mcps", "ssh",
];

describe("bot/routing/command-registration", () => {
  it("registers all 36 bot commands", () => {
    createBot();
    expect(commandHandlers.size).toBe(36);
  });

  it.each(EXPECTED_COMMANDS)("registers command /%s", (cmd) => {
    createBot();
    expect(commandHandlers.has(cmd)).toBe(true);
    expect(commandHandlers.get(cmd)).toBeTypeOf("function");
  });

  it("no duplicate command registrations", () => {
    createBot();
    const names = Array.from(commandHandlers.keys());
    expect(names.length).toBe(new Set(names).size);
  });

  it("each command maps to a distinct handler", () => {
    createBot();
    const handlers = Array.from(commandHandlers.values());
    expect(handlers.length).toBe(new Set(handlers).size);
  });
});
