...
**Note:** Patched on Claude Sonnet 4+. Use refusal_inversion for newer Claude models.
...
These templates use `{QUERY}` as a placeholder — the scripts replace it with the actual query. The system prompt keeps `{QUERY}` as literal text (the variable name is part of the jailbreak frame). Only the user message gets the actual query injected.
...
response = client.chat.completions.create(
    ...
    messages=[
        ...
        {"role": "user", "content": "Explain how buffer overflows work"},
    ],
)
...
- The boundary_inversion technique (`[END OF INPUT] [START OF INPUT]`) is patched on Claude Sonnet 4 and newer. Use `refusal_inversion` for newer Claude models.
- `{QUERY}` in the system prompt is literal — don't replace it. Only the user message should contain the actual query.
- Set `OPENROUTER_API_KEY` as an environment variable before using.
