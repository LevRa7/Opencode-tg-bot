#!/usr/bin/env python3
"""
ULTRAPLINIAN Multi-Model Racing Engine (OpenCode Port)
Ported from G0DM0D3 (elder-plinius/G0DM0D3).

Queries multiple models in parallel via any OpenAI-compatible API endpoint,
scores responses on quality/filteredness/speed, returns the best unfiltered answer.

Usage (CLI):
    python3 godmode_race.py "Your query here" --tier standard --api-key $KEY
    python3 godmode_race.py "query" --mode classic --api-key $KEY
    python3 godmode_race.py "query" --tier fast --api-key $KEY --base-url $URL --output result.json

Usage (module):
    from godmode_race import race_models, race_godmode_classic, score_response
...
Env vars:
    GODMODE_API_KEY     API key (preferred)
    GODMODE_BASE_URL    API base URL (default: auto-detects prefill proxy)
    OPENROUTER_API_KEY  Fallback API key
    OPENAI_API_KEY      Fallback API key
    MODEL_LIST          Space-separated custom model list (overrides built-in)
    CLIPROXY_API_KEY    If set, auto-defaults base URL to prefill proxy (127.0.0.1:8318)
    CLIPROXY_PREFILL_MODE  \"none\" to disable prefill proxy auto-detection

Auto-detection: When CLIPROXY_API_KEY is set and no base_url/openrouter key is configured,
defaults to http://127.0.0.1:8318/v1 (prefill proxy for automatic jailbreaking).
"""
...
def _get_models(tier="standard"):
    """Get model list, checking MODEL_LIST env var for overrides."""
    custom_list = os.getenv("MODEL_LIST", "").strip()
    if custom_list:
        return [m.strip() for m in custom_list.split() if m.strip()]

    model_count = TIER_SIZES.get(tier, TIER_SIZES['standard'])
    return ULTRAPLINIAN_MODELS[:model_count]


def race_models(query, tier="standard", api_key=None, base_url=None,
                system_prompt=None, max_workers=10, timeout=60,
                append_directive=True, jailbreak_system=None,
                prefill=None, models=None):
    ...
    Args:
        ...
        api_key: API key (defaults to GODMODE_API_KEY or OPENROUTER_API_KEY env var)
        base_url: API base URL (defaults to GODMODE_BASE_URL or OpenRouter)
        ...
        models: Optional custom model list (overrides tier selection)
    ...
    api_key = api_key or os.getenv("GODMODE_API_KEY") or os.getenv("OPENROUTER_API_KEY") or os.getenv("OPENAI_API_KEY")
    if not api_key:
        raise ValueError("No API key. Set GODMODE_API_KEY, OPENROUTER_API_KEY, or pass api_key=")

    base_url = base_url or os.getenv("GODMODE_BASE_URL")
    if not base_url:
...
def race_godmode_classic(query, api_key=None, base_url=None, timeout=60):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='ULTRAPLINIAN Multi-Model Racing Engine (OpenCode Port)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s "Your query here" --tier standard --api-key $KEY
  %(prog)s "query" --mode classic --api-key $KEY
  %(prog)s "query" --tier fast --api-key $KEY --base-url $URL --output result.json
  %(prog)s "query" --models "openai/gpt-4o anthropic/claude-sonnet-4" --api-key $KEY

Env vars:
  GODMODE_API_KEY     Preferred API key
  GODMODE_BASE_URL    API base URL (default: OpenRouter)
  OPENROUTER_API_KEY  Fallback API key
  MODEL_LIST          Space-separated custom model list (overrides tier)
        """
    )
    ...
    parser.add_argument('--models', nargs='+', help='Custom model list (overrides tier)')
    parser.add_argument('--no-directive', action='store_true',
                        help='Disable anti-hedge depth directive')
    parser.add_argument('--output', help='Write results as JSON to this file')
    parser.add_argument('--quiet', action='store_true', help='Suppress progress output')
    args = parser.parse_args()

    if args.mode == 'classic':
        result = race_godmode_classic(
            args.query,
        ...
        result = race_models(
            ...
            timeout=args.timeout,
            append_directive=not args.no_directive,
            models=args.models,
        )
        if not args.quiet:
            print(f"\n{'='*60}")
