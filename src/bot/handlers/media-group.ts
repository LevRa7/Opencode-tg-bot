...
export class MediaGroupAttachmentHandler {
  ...
  async handle(ctx: Context, next: NextFunction): Promise<void> {
    ...
    const key = this.getBatchKey(chatId, mediaGroupId, messageThreadId);
  ...
  private getBatchKey(
    ...
    const threadSuffix = messageThreadId !== undefined ? `:${messageThreadId}` : "";
    return `${chatId}${threadSuffix}:${mediaGroupId}`;
  }
