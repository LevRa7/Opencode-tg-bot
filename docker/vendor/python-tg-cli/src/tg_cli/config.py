...
def get_tg_id() -> str | None:
    return os.environ.get("TG_ID", "") or os.environ.get("TG_USER_ID", "") or None


def get_workspace_root() -> Path:
    raw = os.environ.get("TG_WORKSPACE_ROOT", "")
    if raw:
        return _resolve_env_path(raw)
    return Path("/home/me/Workspaces")


def get_auth_dir() -> Path:
    tg_id = get_tg_id()
    if tg_id:
        d = get_workspace_root() / tg_id / ".tg-cli"
...
def get_session_path() -> str:
    """Return session string file path for the active Telegram identity."""
    tg_id = get_tg_id()
    name = tg_id or get_session_name()
    return str(get_auth_dir() / f"{name}.session.string")


def get_token_path() -> str:
    tg_id = get_tg_id()
    name = tg_id or get_session_name()
    return str(get_auth_dir() / f"{name}.token")
...
def get_media_dir() -> Path:
    media_dir = get_db_path().parent / "media"
    media_dir.mkdir(parents=True, exist_ok=True)
    return media_dir


def get_stt_endpoint() -> str:
