...
const getSubagentTopicsEnabledMock = vi.hoisted(() => vi.fn(() => false));
const getSubagentTopicAutoDeleteMinutesMock = vi.hoisted(() => vi.fn(() => 10));
...
describe("bot/index local file follow-up orchestration", () => {
  ...
  it("holds the final answer behind durable tool publication ordering when tool visibility resolves late", async () => {
    vi.resetModules();
    capturedEventCallbacksByDirectory.clear();
    sendMessageMock.mockClear();
    sendMessageDraftMock.mockClear();
    ...
    const toolVisibilityDeferred = createDeferred<boolean>();
    ...
    vi.doMock("../../src/settings/manager.js", () => ({
      ...
      getHideToolCallMessages: vi.fn(() => toolVisibilityDeferred.promise as unknown as boolean),
      getHideToolFileMessages: vi.fn(() => false),
      getUserLocale: vi.fn(() => "en"),
      isMessageStreamingEnabled: vi.fn(() => false),
    }));
    ...
    emit({
      ...
      properties: {
        info: {
          id: "message-tool-durable-order-1",
          sessionID: "session-1",
          role: "assistant",
          time: { created: 1_000 },
        },
    ...
    emit({
      ...
      properties: {
        part: {
          id: "tool-durable-order-part-1",
          sessionID: "session-1",
          messageID: "message-tool-durable-order-1",
          type: "tool",
          callID: "call-bash-durable-order-1",
          tool: "bash",
          state: {
            status: "completed",
            title: "Check tool ordering",
            input: {
              command: "pwd",
              description: "Check tool ordering",
            },
            metadata: {},
    ...
    emit({
      ...
      properties: {
        part: {
          id: "text-tool-durable-order-part-1",
          sessionID: "session-1",
          messageID: "message-tool-durable-order-1",
          type: "text",
          text: "Final answer should wait for the tool publication.",
          time: { start: 1_300 },
        },
      },
    } as unknown as Event);
  ...
  it("does not block final answer delivery when tool visibility lookup rejects", async () => {
    vi.resetModules();
    capturedEventCallbacksByDirectory.clear();
    sendMessageMock.mockClear();
    sendMessageDraftMock.mockClear();
    ...
    vi.doMock("../../src/settings/manager.js", () => ({
      ...
      getThinkingClearMode: vi.fn(() => false),
      getHideThinkingMessages: vi.fn(() => false),
      getHideToolCallMessages: vi.fn(async () => {
        throw new Error("tool visibility unavailable");
      }),
      getHideToolFileMessages: vi.fn(() => false),
      getUserLocale: vi.fn(() => "en"),
      isMessageStreamingEnabled: vi.fn(() => false),
    ...
    await promptHandler({
      message: {
        text: "tool visibility rejection should not block final delivery",
        chat: { id: 123 },
        message_thread_id: 1,
      },
      chat: { id: 123, type: "private" },
    ...
    emit({
      ...
      properties: {
        info: {
          id: "message-tool-visibility-rejection-1",
          sessionID: "session-1",
          role: "assistant",
          time: { created: 1_000 },
        },
    ...
    emit({
      ...
      properties: {
        part: {
          id: "tool-visibility-rejection-part-1",
          sessionID: "session-1",
          messageID: "message-tool-visibility-rejection-1",
          type: "tool",
          callID: "call-bash-visibility-rejection-1",
          tool: "bash",
          state: {
            status: "completed",
            title: "Check tool visibility failure handling",
            input: {
              command: "pwd",
              description: "Check tool visibility failure handling",
            },
            metadata: {},
            time: { start: 1_100, end: 1_200 },
          },
    ...
    emit({
      ...
      properties: {
        part: {
          id: "text-tool-visibility-rejection-part-1",
          sessionID: "session-1",
          messageID: "message-tool-visibility-rejection-1",
          type: "text",
          text: "Final answer should still be delivered.",
          time: { start: 1_300 },
        },
      },
    } as unknown as Event);
    ...
    expect(String(sendMessageMock.mock.calls[0]?.[1] ?? "")).toContain(
      "Final answer should still be delivered.",
    );
  });

  it("holds the final answer behind durable subagent publication ordering when card rendering resolves late", async () => {
    try {
      vi.resetModules();
      capturedEventCallbacksByDirectory.clear();
      sendMessageMock.mockClear();
      ...
      const renderedCardsDeferred = createDeferred<string>();
      ...
      vi.doMock("../../src/summary/subagent-formatter.js", () => ({
        renderSubagentCards: vi.fn(() => renderedCardsDeferred.promise),
      }));
      ...
      await promptHandler({
        message: {
          text: "subagent durable ordering",
          chat: { id: 123 },
          message_thread_id: 1,
        },
        chat: { id: 123, type: "private" },
      ...
      emit({
        ...
        properties: {
          part: {
            id: "subtask-durable-order-1",
            sessionID: "session-1",
            messageID: "root-message-durable-order-1",
            type: "subtask",
            prompt: "Inspect artifact",
            description: "Inspect /tmp/report.txt",
            agent: "explore",
      ...
      emit({
        ...
        properties: {
          info: {
            id: "message-subagent-durable-order-1",
            sessionID: "session-1",
            role: "assistant",
            time: { created: 2_000 },
          },
      ...
      emit({
        ...
        properties: {
          part: {
            id: "text-subagent-durable-order-part-1",
            sessionID: "session-1",
            messageID: "message-subagent-durable-order-1",
            type: "text",
            text: "Final answer should wait for the subagent publication.",
            time: { start: 2_100 },
          },
        },
      } as unknown as Event);
      ...
      renderedCardsDeferred.resolve("Task: Inspect artifact\nAgent: explore");

      await vi.waitFor(() => expect(sendMessageMock).toHaveBeenCalledTimes(2));
      expect(String(sendMessageMock.mock.calls[0]?.[1] ?? "")).toContain("Task: Inspect artifact");
      expect(String(sendMessageMock.mock.calls[1]?.[1] ?? "")).toContain(
        "Final answer should wait for the subagent publication.",
      );
    } finally {
      vi.doUnmock("../../src/summary/subagent-formatter.js");
    }
  });

  it("routes child-session final output to a dedicated topic while root output stays on the parent route", async () => {
    vi.resetModules();
    capturedEventCallbacksByDirectory.clear();
    sendMessageMock.mockClear();
    ...
    emit({
      ...
      properties: {
        part: {
          id: "subtask-topic-route-1",
          sessionID: "session-1",
          messageID: "root-topic-route-1",
          type: "subtask",
          prompt: "Inspect artifact",
    ...
    emit({
      ...
      properties: {
        part: {
          id: "root-text-topic-route-1",
          sessionID: "session-1",
          messageID: "root-message-topic-route-1",
          type: "text",
          text: "Root reply stays in parent thread.",
          time: { start: 1_150 },
        },
      },
    } as unknown as Event);
    ...
    emit({
      ...
      properties: {
        info: {
          id: "child-message-topic-route-1",
          sessionID: "child-session-topic-1",
          role: "assistant",
          time: { created: 1_300 },
        },
      },
    ...
    emit({
      ...
      properties: {
        part: {
          id: "child-text-topic-route-1",
          sessionID: "child-session-topic-1",
          messageID: "child-message-topic-route-1",
          type: "text",
          text: "Child output goes to dedicated topic only.",
          time: { start: 1_350 },
        },
      },
    } as unknown as Event);
    ...
    await vi.waitFor(() =>
      expect(
        sendMessageMock.mock.calls.some(
          (call) =>
            call[1] === "Child output goes to dedicated topic only." &&
    ...
    expect(
      sendMessageMock.mock.calls.filter(
        (call) => String(call[1]) === "Child output goes to dedicated topic only.",
      ),
    ).toHaveLength(1);
  });
  ...
  it("keeps a child topic alive until the final child answer is delivered when session.idle arrives first", async () => {
    ...
    try {
      ...
      sendMessageMock.mockImplementationOnce(() => deferredChildFinalMessage.promise);
      ...
      await promptHandler({
        message: {
          text: "route child topic with delayed final",
          chat: { id: 123, is_forum: true },
          message_thread_id: 1,
        },
        chat: { id: 123, type: "supergroup", is_forum: true },
      ...
      emit({
        ...
        properties: {
          info: {
            id: "child-session-topic-delayed-1",
            parentID: "session-1",
            title: "Inspect artifact (@explore subagent)",
            directory: "/repo",
            time: { created: 1_000, updated: 1_000 },
      ...
      emit({
        ...
        properties: {
          part: {
            id: "subtask-topic-delayed-1",
            sessionID: "session-1",
            messageID: "root-topic-delayed-1",
            type: "subtask",
            prompt: "Inspect artifact",
            description: "Inspect artifact",
            agent: "explore",
      ...
      emit({
        ...
        properties: {
          part: {
            id: "child-text-topic-delayed-1",
            sessionID: "child-session-topic-delayed-1",
            messageID: "child-message-topic-delayed-1",
            type: "text",
            text: "Deliver this child answer before deleting the topic.",
            time: { start: 1_350 },
          },
        },
      } as unknown as Event);
      ...
      emit({
        type: "session.idle",
        properties: { sessionID: "child-session-topic-delayed-1" },
      } as unknown as Event);
      ...
      expect(sendMessageMock).toHaveBeenCalledWith(
        123,
        "Deliver this child answer before deleting the topic.",
        expect.objectContaining({
      ...
      deferredChildFinalMessage.resolve({ message_id: 42 });
  ...
  it("does not schedule child topic deletion when final child delivery fails", async () => {
    vi.useFakeTimers();
    ...
    try {
      ...
      getSubagentTopicAutoDeleteMinutesMock.mockReturnValue(1);
      createForumTopicMock.mockResolvedValue({ messageThreadId: 321 });
      sendMessageMock.mockRejectedValueOnce(new Error("child final send failed"));
      ...
      await promptHandler({
        message: {
          text: "route child topic with failed final",
          chat: { id: 123, is_forum: true },
          message_thread_id: 1,
        },
        chat: { id: 123, type: "supergroup", is_forum: true },
      ...
      emit({
        ...
        properties: {
          info: {
            id: "child-session-topic-failed-1",
            parentID: "session-1",
            title: "Inspect artifact (@explore subagent)",
            directory: "/repo",
            time: { created: 1_000, updated: 1_000 },
      ...
      emit({
        ...
        properties: {
          part: {
            id: "subtask-topic-failed-1",
            sessionID: "session-1",
            messageID: "root-topic-failed-1",
            type: "subtask",
            prompt: "Inspect artifact",
            description: "Inspect artifact",
            agent: "explore",
      ...
      emit({
        ...
        properties: {
          info: {
            id: "child-message-topic-failed-1",
            sessionID: "child-session-topic-failed-1",
            role: "assistant",
            time: { created: 1_300 },
          },
        },
      ...
      emit({
        ...
        properties: {
          part: {
            id: "child-text-topic-failed-1",
            sessionID: "child-session-topic-failed-1",
            messageID: "child-message-topic-failed-1",
            type: "text",
            text: "This child final answer should fail to send.",
            time: { start: 1_350 },
          },
        },
      } as unknown as Event);
      emit({
        type: "session.idle",
        properties: { sessionID: "child-session-topic-failed-1" },
      } as unknown as Event);
      emit({
        type: "message.updated",
        properties: {
  ...
  it("falls back to the parent route when dedicated child topic routing is unavailable", async () => {
    ...
    emit({
      ...
      properties: {
        part: {
          id: "subtask-fallback-route-1",
          sessionID: "session-1",
          messageID: "root-fallback-route-1",
          type: "subtask",
    ...
    emit({
      ...
      properties: {
        info: {
          id: "child-session-fallback-route-1",
          parentID: "session-1",
          title: "Fallback helper (@explore subagent)",
          directory: "/repo",
    ...
    emit({
      ...
      properties: {
        info: {
          id: "child-message-fallback-route-1",
          sessionID: "child-session-fallback-route-1",
          role: "assistant",
          time: { created: 2_100 },
        },
      },
    ...
    emit({
      ...
      properties: {
        part: {
          id: "child-text-fallback-route-1",
          sessionID: "child-session-fallback-route-1",
          messageID: "child-message-fallback-route-1",
          type: "text",
          text: "Fallback child output stays on the parent route.",
          time: { start: 2_150 },
        },
      },
    } as unknown as Event);
    ...
    await vi.waitFor(() =>
      expect(
        sendMessageMock.mock.calls.some(
          (call) =>
            call[1] === "Fallback child output stays on the parent route." &&
  ...
  it("waits to send the footer until the durable final answer materializes and uses completion time for elapsed", async () => {
    vi.mocked(isMessageStreamingEnabled).mockReturnValue(false);

    const deferredFinalMessage = createDeferred<{ message_id: number }>();
    sendMessageMock
      .mockImplementationOnce(() => deferredFinalMessage.promise)
    ...
    emit({
      ...
      properties: {
        info: {
          id: "message-durable-footer-1",
          sessionID: "session-1",
          role: "assistant",
          time: { created: currentNow },
        },
      },
    } as unknown as Event);
    ...
    emit({
      ...
      properties: {
        part: {
          id: "text-durable-footer-1",
          sessionID: "session-1",
          messageID: "message-durable-footer-1",
          type: "text",
          text: "Durable final answer.",
          time: { start: currentNow },
        },
      },
    } as unknown as Event);
    ...
    emit({
      ...
      properties: {
        info: {
          ...
          modelID: "gpt-5.4",
          providerID: "openai",
          time: { created: 1_000, completed: currentNow },
        },
      },
    } as unknown as Event);
    ...
    await vi.waitFor(() => expect(sendMessageMock).toHaveBeenCalledTimes(1));
    expect(String(sendMessageMock.mock.calls[0]?.[1] ?? "")).toContain("Durable final answer.");
    expect(
      sendMessageMock.mock.calls.some((call) =>
    ...
    deferredFinalMessage.resolve({ message_id: 42 });

    await vi.waitFor(() => expect(sendMessageMock).toHaveBeenCalledTimes(2));
    expect(sendMessageMock).toHaveBeenNthCalledWith(
      2,
      123,
      expect.stringContaining("📋 Plan Mode · 🤖 openai/gpt-5.4 · 🕒 1.5s"),
      expect.objectContaining({ message_thread_id: 1 }),
    );
  });

  it("does not enqueue a footer for an empty completion with no visible final answer", async () => {
    const bot = createBot() as unknown as FakeBot;
    const textHandlers = bot.onHandlers.filter((entry) => entry.event === "message:text");
    const promptHandler = textHandlers[textHandlers.length - 1]?.handler;
    ...
    await promptHandler({
      message: {
        text: "empty completion should not produce footer",
        chat: { id: 123 },
        message_thread_id: 1,
      },
      chat: { id: 123, type: "private" },
  ...
  it("passes the declared sendApi contract into streamThinkingBlocks for active reasoning updates", async () => {
    ...
    expect(thinkingCall?.logicalMessageId).toBe("message-reasoning-draft-api-1");
    expect(thinkingCall?.target).toEqual({ chatId: 123, messageThreadId: 1 });

    vi.doUnmock("../../src/bot/utils/thinking-block-stream.js");
  });

  it("keeps the final assistant completion path alive when final thinking publication fails", async () => {
    let nextMessageId = 100;
    let sendMessageCallCount = 0;
    sendMessageMock.mockImplementation(async () => {
      sendMessageCallCount += 1;
      if (sendMessageCallCount === 2) {
        throw new Error("thinking final publish failed");
      }
    ...
    try {
      ...
      const ctx = {
        message: {
          text: "reasoning final publish failure should not stop completion",
          chat: { id: 123 },
          message_thread_id: 1,
        },
        chat: { id: 123, type: "private" },
      ...
      emit({
        ...
        properties: {
          info: {
            id: "message-reasoning-final-failure-1",
            sessionID: "session-1",
            role: "assistant",
            time: { created: Date.now() },
          },
      ...
      emit({
        ...
        properties: {
          part: {
            id: "reasoning-final-failure-part-1",
            sessionID: "session-1",
            messageID: "message-reasoning-final-failure-1",
            type: "reasoning",
            text: "Planning the answer before the final publication fails.",
            time: { start: Date.now() },
          },
        },
      } as unknown as Event);
      ...
      emit({
        ...
        properties: {
          part: {
            id: "text-final-failure-part-1",
            sessionID: "session-1",
            messageID: "message-reasoning-final-failure-1",
            type: "text",
            text: "The assistant answer still needs to complete.",
            time: { start: Date.now() },
          },
        },
      } as unknown as Event);
  ...
  it("continues assistant partial streaming when live thinking delivery fails", async () => {
    vi.resetModules();
    capturedEventCallbacksByDirectory.clear();
    sendMessageMock.mockClear();
    sendMessageDraftMock.mockClear();
    ...
    const streamThinkingBlocksSpy = vi.fn(async () => {
      throw new Error("thinking live failed");
    });
    ...
    const ctx = {
      message: {
        text: "thinking live failure should not block assistant streaming",
        chat: { id: 123 },
        message_thread_id: 1,
      },
      chat: { id: 123, type: "private" },
    ...
    emit({
      ...
      properties: {
        info: {
          id: "message-reasoning-live-failure-1",
          sessionID: "session-1",
          role: "assistant",
          time: { created: Date.now() },
        },
    ...
    emit({
      ...
      properties: {
        part: {
          id: "reasoning-live-failure-part-1",
          sessionID: "session-1",
          messageID: "message-reasoning-live-failure-1",
          type: "reasoning",
          text: "Thinking live delivery fails here.",
          time: { start: Date.now() },
        },
      },
    } as unknown as Event);
    ...
    emit({
      ...
      properties: {
        part: {
          id: "text-live-failure-part-1",
          sessionID: "session-1",
          messageID: "message-reasoning-live-failure-1",
          type: "text",
          text: "Assistant text still streams.",
          time: { start: Date.now() },
        },
      },
    } as unknown as Event);

    const hasStreamedAssistantText = () =>
      sendMessageMock.mock.calls.some((call) =>
        String(call[1] ?? "").includes("Assistant text still streams."),
      );

    await vi.waitFor(() => expect(streamThinkingBlocksSpy).toHaveBeenCalled());
    await vi.waitFor(() => expect(hasStreamedAssistantText()).toBe(true));
  ...
  it("clears durable final ordering state when session error loses routing", async () => {
    vi.resetModules();
    capturedEventCallbacksByDirectory.clear();
    sendMessageMock.mockClear();
    sendMessageDraftMock.mockClear();
    ...
    const clearDurableOrderingSessionSpies: Array<ReturnType<typeof vi.fn>> = [];
    ...
    vi.doMock("../../src/bot/delivery/session-delivery-orchestrator.js", () => ({
      SessionDeliveryOrchestrator: class {
        private readonly clearSessionSpy = vi.fn();

        constructor(_options?: unknown) {
          clearDurableOrderingSessionSpies.push(this.clearSessionSpy);
        }
        ...
        clearSession(sessionId: string): void {
          this.clearSessionSpy(sessionId);
        }
    ...
    const { createBot: createIsolatedBot, routingBySessionId } =
      await import("../../src/bot/index.js");
    expect(clearDurableOrderingSessionSpies).toHaveLength(2);
    const bot = createIsolatedBot() as any;
    const textHandlers = bot.onHandlers.filter(
      (entry: { event: string | string[] }) => entry.event === "message:text",
    );
    ...
    await promptHandler({
      message: {
        text: "clear durable ordering on missing-routing session error",
        chat: { id: 123 },
        message_thread_id: 1,
      },
      chat: { id: 123, type: "private" },
    ...
    expect(clearDurableOrderingSessionSpies[1]).not.toHaveBeenCalled();
    ...
    await vi.waitFor(() =>
      expect(clearDurableOrderingSessionSpies[1]).toHaveBeenCalledWith("session-1"),
    );
  });
});
