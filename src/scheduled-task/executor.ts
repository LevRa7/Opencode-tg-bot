...
function normalizeScheduledTaskErrorMessage(message: string | null): string | null {
  if (!message) {
    return null;
  }

  const prefix = `${EXECUTION_TIMEOUT_ERROR_PREFIX}: `;

  if (message.startsWith(prefix)) {
    return message;
  }

  return `${EXECUTION_TIMEOUT_ERROR_PREFIX}: ${message}`;
}

function createExecutionTimeoutMessage(): string {
  const executionTimeoutMs = getExecutionTimeoutMs();
  return `${EXECUTION_TIMEOUT_ERROR_PREFIX} (${executionTimeoutMs / 1000}s). Check ${MODELS_DOCS_URL} for model configurations.`;
}

function getExecutionTimeoutMs(): number {
  return Math.max(60_000, config.bot.scheduledTaskExecutionTimeoutMinutes * 60 * 1000);
}
...
async function waitForScheduledTaskResult(
  ...
  while (true) {
    ...
    if (currentSession && currentSession.type !== "busy") {
      ...
      if (idlePollsWithoutResult >= MAX_IDLE_POLLS_WITHOUT_RESULT) {
        throw new Error("Scheduled task assistant stopped responding");
      }
    }
