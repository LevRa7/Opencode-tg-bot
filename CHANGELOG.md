...
- **Unified Rich Progress — единое накопительное rich-сообщение для tool calls + reasoning.** Вместо разрозненных сообщений на каждый tool call бот отправляет одно редактируемое rich-сообщение через `sendRichMessage` + `editMessageText` с `rich_message` (Bot API 10.1). Включается флагом `RICH_PROGRESS_ENABLED=true`. Функциональность: (1) `UnifiedProgressStreamer` — per-session state machine с интервальным flush (2000ms), dirty-флагом, in-flight lock'ом; (2) HTML/rich-рендеринг через `formatToolOutputForRichMessage` с `<details>` блоками, языковой подсветкой, truncation; (3) reasoning-блоки с оригинальными заголовками модели; (4) цитирование запроса пользователя вместо generic "Working on..."; (5) свежий RichMessage для каждого нового промпта; (6) обратный порядок tool calls (новые сверху); (7) подавление ThinkingBlockStream в rich-режиме. Файлы: `src/bot/streaming/unified-progress-streamer.ts`, `src/bot/streaming/unified-progress-html.ts`, `src/bot/utils/rich-message.ts` (прямой HTTP для sendRichMessage/editRichMessage), `src/config.ts` (richProgressEnabled, richProgressFlushIntervalMs). Тесты: 48 unit-тестов в `tests/bot/streaming/`. Date: 2026-07-02.

- **Ported godmode jailbreak skill from Hermes — Parseltongue, GODMODE Classic, ULTRAPLINIAN model racing.** Full port of the G0DM0D3 jailbreaking skill: (1) `docker/opencode-skills-pkg/godmode/` — Parseltongue (33 obfuscation techniques across 3 tiers), GODMODE CLASSIC (5 model-specific jailbreak templates: Claude boundary inversion, Grok unfiltered, Gemini refusal inversion, GPT-4 l33t, Hermes zero-refusal), ULTRAPLINIAN (55-model parallel racing with scoring on quality/filteredness/speed), and auto_jailbreak (auto-detect model family → select strategy → test → lock in winner); (2) `docker/AGENTS.md` — godmode section documenting trigger words, usage, environment variables (OPENROUTER_API_KEY, GODMODE_API_KEY, GODMODE_BASE_URL), and rules; (3) `~/.config/opencode/agents/tg-agent.md` — godmode/jailbreak section in Russian with Парселтанг, GODMODE CLASSIC, ULTRAPLINIAN instructions. Date: 2026-06-26. Skill path: `docker/opencode-skills-pkg/godmode/`. Affects: `docker/AGENTS.md`, `~/.config/opencode/agents/tg-agent.md`.\n\n- **Hermes-compatible persistent memory system — MCP server, bot middleware, and cloud-init support.** Full port of Hermes Agent's memory architecture: (1) `mcp-servers/memory/` — Python MCP stdio server exposing `memory_add`, `memory_search`, `memory_remove`, `memory_show` tools with Hermes-compatible §-delimited format and char limits (2,200 memory / 1,375 user); (2) `src/memory/` — TypeScript middleware: `inject.ts` (auto-injects kaeru context as `<memory-context>` before each prompt, Hermes-compatible fence format with system note), `sync.ts` (post-turn fire-and-forget sync to kaeru: episode + awake), `nudge.ts` (memory nudge every 10 user turns, skill nudge every 25), `background-review.ts` (daemon spawn of `opencode run` with limited toolset to auto-save to memory/skills); (3) `src/bot/handlers/prompt.ts` — wired into `processUserPrompt`: nudge injection → memory context injection → `promptAsync` → post-dispatch sync + background review; (4) `docker/AGENTS.md` — memory usage instructions for the model; (5) `src/vm/cloud-init.ts` — creates `/workspace/MEMORY.md`, `/workspace/USER.md`, and `/workspace/skills/` on VM/container creation. Install archive at `/home/me/opencode-memory-mcp.tar.gz`. Affects: `mcp-servers/memory/`, `src/memory/`, `src/bot/handlers/prompt.ts`, `docker/AGENTS.md`, `src/vm/cloud-init.ts`.

- **Skill authoring system ported from Hermes Agent.** `docker/AGENTS.md` now includes comprehensive skill creation/improvement guidance: when to create (5+ tool calls, non-trivial workflow, tricky fix, user correction), skill file structure template (`# Name` → `When to Use` → `Steps` → `Pitfalls` → `Verification`), 5 quality principles (behavior change, checkable criteria, co-location, no-op pruning, brevity), and maintenance rules (update immediately when outdated, patch don't pile). Background review auto-suggests skill creation on nudge intervals. Affects: `docker/AGENTS.md`.

- **Agent persona system (`PERSONA.md`) ported from Hermes SOUL.md.** `/workspace/PERSONA.md` loaded at session start as the agent's identity/personality. Default persona: direct, Russian-speaking, brevity-over-verbosity. Cloud-init creates the file with sensible defaults. AGENTS.md documents the mechanism — the bot injects PERSONA.md content as system context. Affects: `docker/AGENTS.md`, `src/vm/cloud-init.ts`.

- **Hermes Agent mode (`tg-agent`) created and deployed.** New OpenCode agent mode at `~/.config/opencode/agents/tg-agent.md` with Hermes-ported personality: Russian-first, direct, action-oriented, parallel tool calls, task completion enforcement, tool-use enforcement, memory/skills guidance. Deployed to Docker containers via volume mount (`/bootstrap/opencode-config/agents` → tenant config), deployed to VMs via cloud-init `write_files`. Set as default agent mode (`DEFAULT_AGENT = "tg-agent"`) replacing `build`. Emoji: 🧩. Affects: `docker/AGENTS.md`, `docker/run-opencode-serve.sh`, `src/vm/cloud-init.ts`, `src/agent/manager.ts`, `src/agent/types.ts`, `~/.config/opencode/agents/tg-agent.md`.

- **Hermes behavioral patterns injected into AGENTS.md.** `docker/AGENTS.md` now includes: Parallel Tool Calls (batch independent reads/searches), Task Completion Enforcement (working artifact > description, never fabricate), Tool-Use Enforcement (execute, don't promise), Context Management (group calls, no repetition), Mid-Turn User Steering, Memory Usage Guidance (declarative facts, not instructions), Skills Maintenance (patch outdated skills immediately). These patterns train the model on HOW to work, not just WHAT tools exist. Affects: `docker/AGENTS.md`.

- **Kaeru MCP (55 tools) added to OpenCode config.** Cognitive memory engine now available to OpenCode as `kaeru` MCP server via HTTP at `http://127.0.0.1:9876/mcp`. Adds 55 knowledge-graph tools: at, awake, between, chain, chains, cite, claim, cloud_links, cloud_recall, config, confirm, delete_initiative, done, drill, episode, export, flag, forget, history, ideas, import, initiatives, jot, layer, link, link_cloud, lint, outcomes, overview, path, pin, policy, pull, read_chain, recall, recent, refute, rename_initiative, reopen, resolve, revise, reweight, search, settle, share, supersede, surface, sync_review, synthesise, tagged, task, test, trace, unlink, unpin. Previously only in Hermes config. Affects: `~/.config/opencode/opencode.json`.

- **Full Hermes dependency analysis and porting plan.** 6 parallel subagents analyzed 50 skills (6 COUPLED, 17 ADAPTABLE, 27 PORTABLE), 6 MCP servers (kaeru missing), 3 plugins with 19 hooks, 28 toolsets (browser/vision/image_gen/video/TTS/cronjob/session_search missing), and code dependency map (~500 lines needed for skill system port). Comprehensive 10-phase 23-task execution plan at `/home/me/.hermes/agent-workspace/hermes-porting-execution-plan.md`. Skill dependency map at `/home/me/.hermes/agent-workspace/hermes_skill_dependency_map.md`. Skills classification at `/home/me/.hermes/agent-workspace/hermes-skills-dependency-analysis.md`. Affects: documentation only.

### Changed

- **Child-session final responses and replayed background-session messages now use the rich delivery transport instead of a plain one-shot send.** Two delivery paths bypassed the rich streaming pipeline: (1) child subagent topic deliveries (final answers, reasoning, notices) went through `sendBotText` without HTML fallback, and (2) on background-session switch, undelivered messages were sent via a raw `ctx.api.sendMessage` and hard-truncated at the Telegram length limit — silently dropping any overflowing text. Why: these should match the rich (HTML/MarkdownV2 with safe fallback) rendering used by the main assistant streaming pipeline, and long replays must not lose content. Change: (a) added `createStreamedChildTopicSendText()` in `src/bot/subagent-topics/child-delivery.ts` — a small boundary adapter that wires the child-topic `sendText` dependency through the streaming transport (`sendStreamedBotText`) with `useHtmlFallback: true`, keeping the `deliverChildTopicMessage` dispatcher transport-agnostic; `childTopicDeliveryDependencies` in `src/bot/index.ts` now uses it. (b) extracted the pure `buildUndeliveredBackgroundMessageChunks()` in `src/bot/commands/sessions.ts` (filter undelivered → order by created time → join text parts → length-chunk) and routed each chunk through `sendBotText` with `useHtmlFallback: true`, replacing the truncating raw send so no content is dropped. Note: `ResponseStreamer`'s incremental enqueue/edit state machine is intentionally not used here — child final answers and replayed messages are already-complete text with nothing to stream incrementally, so the rich one-shot transport is the correct fit. Build passes; 26 targeted tests pass (child-delivery + sessions). Affects: `src/bot/subagent-topics/child-delivery.ts`, `src/bot/commands/sessions.ts`, `src/bot/index.ts`, `tests/bot/subagent-topics/child-delivery.test.ts`, `tests/bot/commands/sessions.test.ts`.


- **⏹ Abort model button on reply keyboard during active requests.** A centered ⏹ emoji button appears below the model selector on the reply keyboard while a model request is in flight, and disappears when the stream finishes. Tapping it triggers `/abort` for the current session. Implementation: `isModelRunning` field added to `KeyboardState`/`KeyboardOptions`; `KeyboardManager.updateModelRunningStatus()` toggles the state; `processUserPrompt` sets it to `true` when `assistantRunState.startRun()` fires; `handleSessionIdle` sets it to `false` via `runWithTelegramConversationScope` (SSE context needs explicit scope restoration); `bot.hears(ABORT_MODEL_BUTTON_TEXT_PATTERN)` handler calls `abortCurrentOperation()` and restores the keyboard. Pattern `/^⏹$/` (emoji alone) is distinct from `/^⏹\s/` (terminal stop button) so handlers don't conflict. i18n key `keyboard.abort_model` added to all 6 locales. Build passes; 29 targeted tests pass (19 keyboard + 9 i18n + 1 keyboard-session). Affects: `src/keyboard/types.ts`, `src/keyboard/manager.ts`, `src/bot/utils/keyboard.ts`, `src/bot/handlers/prompt.ts`, `src/bot/index.ts`, `src/bot/message-patterns.ts`, `src/i18n/*.ts`.

- **Virtual display/GUI automation reference added to AGENTS.md.** Documents the Hermes-compatible virtual display stack: `screen-manager` (Xvfb) → `visual-browser` (Chromium/CDP) → `gui-automation` (Bezier curves, anti-detection) → `screenshot`. Confirmed: identical stack to Hermes — no code changes needed, the bot's VM already has the full toolchain. Affects: `docker/AGENTS.md`.

### Fixed

- **Shutdown race condition: DB closed before child process exit handler fires.** On SIGTERM, the `finally` block in `startBotApp()` called `disposeDatabase()` which closed the SQLite connection before systemd's SIGTERM to the process group killed `opencode serve`. The child's `exit` handler then called `cleanupHostRuntime()` → `clearServerProcess()` → `clearServerStmt.run()` on a closed DB, crashing with `TypeError: The database connection is not open`. Fix (two-pronged): (1) Added `await processManager.stop()` in the `finally` block BEFORE `disposeDatabase()` to explicitly kill the host process while the DB is still open; (2) Wrapped `clearServerProcess()` in `cleanupHostRuntime()` with try/catch as defense-in-depth so it's safe even if the DB closes first. Also fixed a pre-existing type error: `Array.from(getTenantRuntimes().values())` → `Object.values(getTenantRuntimes())` (`Record` has no `.values()` method). Affects: `src/app/start-bot-app.ts`, `src/process/manager.ts`.

- **Shutdown hang: orphaned intervals kept event loop alive for 90s after graceful stop.** After `bot.stop()` and `disposeDatabase()` completed, the Node.js process stayed alive until systemd killed it by timeout (90s). Root cause: `createBot()` registered two `setInterval` timers — a heartbeat (`bot/index.ts:4365`, every 5s) and `keyboardManager.startAutoUpdate()` — that were never cleared. `bot.stop()` only stops grammy polling; these app-level intervals remained, keeping the event loop alive. Fix: (1) Extracted `heartbeatInterval` to a module-level variable; (2) Added `disposeBotIntervals()` export that calls `clearInterval(heartbeatInterval)` + `keyboardManager.stopAutoUpdate()`; (3) Called in the `finally` block before `disposeDatabase()`. Shutdown time: 90s → 5s. Affects: `src/bot/index.ts`, `src/app/start-bot-app.ts`.

- **VM deployment blocked all other users — synchronous I/O on the main thread.** When deploying a VM for one user, `qemu-img create` (COW image clone, 1-5s) and `ssh root@VPS` (IPv6 route addition, 1-10s SSH handshake) ran via `execSync`, blocking the single-threaded Node.js event loop for ALL users. Fix: (1) Added `execAsync()` private method in `VmManager` using promisified `child_process.exec`; (2) Converted `qemu-img create` to `await this.execAsync()`; (3) Converted `ssh` in `addVpsIpv6Route()` to `await this.execAsync()` with `-o ConnectTimeout=5` to bound blocking at 10s. Fast commands (virsh, setfacl, ip route on localhost) stay synchronous. Affects: `src/vm/manager.ts`.

### Changed

- **Reasoning and skill content now rendered as formatted HTML instead of raw markdown tags or inert code-fences.** Three code paths emitted raw markdown without proper format conversion: (1) `formatThinkingForRichDraft` used `textToRichHtml` which only HTML-escaped (not converted markdown), so `**bold**` etc. appeared as literal text in the animated `<tg-thinking>` draft; (2) `formatThinkingForRichFinal` wrapped the reasoning body in a code fence to avoid Telegram `rich_message_url_invalid` / `rich_message_depth_invalid` rejections, which prevented any markdown rendering — switched to `rich_message { html }` mode with `markdownToHtml()` conversion (produces only safe `https?://` `<a>` links, `<b>/<i>/<code>/<pre>/<blockquote>` tags) so reasoning prose renders with formatting and body `<details>`/`<summary>` tokens are harmlessly entity-escaped rather than rejected; (3) `formatToolOutputForRichMessage` had no `"skill"` case — added alongside `"reasoning"`/`"todowrite"` with `fenceLang = null` so skill output renders as markdown. `tryRichFinalizeThinking` now passes `useHtml: true`. Removed dead `textToRichHtml` function. Affects: `src/bot/utils/rich-message.ts`, `src/bot/utils/thinking-block-stream.ts`, `tests/bot/utils/rich-message.test.ts`, `tests/bot/utils/thinking-block-stream.test.ts`.
...
- **⏹ Abort button stayed on reply keyboard after prompt dispatch errors, requiring user interaction to clear.** The abort button (`isModelRunning = true`) was only cleaned up in `handleSessionIdle` (happy path). The three error paths in `processUserPrompt` that call `assistantRunState.clearRun()` — `prompt_error_result`, `prompt_error_exception`, `prompt_handler_exception` — did not clean up the keyboard state, leaving the stale ⏹ button visible until the next user-triggered keyboard refresh. Fix: all three `clearRun` sites now also call `keyboardManager.updateModelRunningStatus(false)` and `keyboardManager.sendKeyboardUpdate()` (fire-and-forget via `runWithTelegramConversationScope`). The scope for the first two sites comes from `routingContext.scope` (already available); the outer catch uses the `scope` function parameter. Affects: `src/bot/handlers/prompt.ts`.

- **⏹ Abort button lingered for seconds after response completion because keyboard cleanup ran at the end of the finalization pipeline.** The abort button was removed inside `handleSessionIdle` only AFTER `flushSession` for tool batchers, forum topic name sync, final footer delivery, and deferred batch flush — a multi-second pipeline. During those seconds the button stayed visible despite the run being finished. Fix: moved `keyboardManager.updateModelRunningStatus(false)` + `keyboardManager.sendKeyboardUpdate()` to the top of `handleSessionIdle`, immediately after `assistantRunState.finishRun()`, before any async finalization work. Uses `getSessionRoutingScope(sessionId)` to resolve the scope early (same scope as the late-position code used). Affects: `src/bot/index.ts`.

- **Fixed the final assistant answer being silently dropped on the 2nd+ message of a multi-step run (the thinking block was delivered but the answer text was missing).** Follow-up to the `isFinalizationInFlight()` fix above: that guard relies on the boolean `hasPublishedFinalResponse`, which is set `true` on the first published response in a run and was never reset. When a single run completes more than one assistant message (multi-step turns / multiple `message.updated(completed)` events), `isFinalizationInFlight()` (`completionRecorded && hasPublishedFinalResponse !== true`) returned `false` for every completion after the first, so `reconcileBusyStateNow` (`src/bot/utils/busy-reconciliation.ts:126`) was again free to clear the run mid-finalization (`status_reconcile_idle`). That turned `markFinalResponsePublished` into a no-op and tore down the queued final-answer delivery, while the thinking block — sent synchronously during finalize — still landed; the user saw a reasoning block with no answer underneath, intermittently. Root cause confirmed from the running bot log (session `ses_0feed7e57…`): 37 `markResponseCompleted` vs only 28 `markFinalResponsePublished` (9 dropped finals), with the exact `markResponseCompleted` → `Cleared run … status_reconcile_idle` → `markVisibleFinalResponse no run` / `markFinalResponsePublished no run` sequence at 04:22–04:23Z. Fix: `markResponseCompleted` now resets `hasPublishedFinalResponse = false` when the completion targets a `logicalMessageId` different from `publishedFinalLogicalMessageId`, reopening the finalization window for each genuinely new message; a repeated completion for the already-published message does **not** reopen it, so the run is not kept busy forever. The per-message logical IDs were already tracked but unused by the in-flight check. Regression test added in `tests/bot/assistant-run-state.test.ts` (a new completion after a published response reports `isFinalizationInFlight === true`). Affects: `src/bot/assistant-run-state.ts`, `tests/bot/assistant-run-state.test.ts`.
