...
const toolMessageBatcher = new ToolMessageBatcher({
  sendText: async (sessionId, text) => {
    ...
    const messageThreadId = getMessageThreadIdForSession(sessionId);
  ...
  sendText: async (sessionId, text) => {
    ...
    const sentMessage = await botInstance.api.sendMessage(
      ...
      withMessageThreadId(
        ...
        getMessageThreadIdForSession(sessionId),
      ),
    );
...
async function ensureEventSubscription(directory: string): Promise<void> {
  ...
  summaryAggregator.setOnQuestion(async (questions, requestID) => {
    ...
    await showCurrentQuestion(
      botInstance.api,
      chatIdInstance,
      currentSession ? getMessageThreadIdForSession(currentSession.id) : undefined,
    );
  });
  ...
  summaryAggregator.setOnPermission(async (request) => {
    ...
    await showPermissionRequest(
      ...
      chatIdInstance,
      request,
      getMessageThreadIdForSession(request.sessionID),
    );
  });
  ...
  summaryAggregator.setOnSessionError(async (sessionId, message) => {
    ...
    await botInstance.api
      .sendMessage(
        ...
        withMessageThreadId(undefined, getMessageThreadIdForSession(sessionId)),
      )
      .catch((err) => {
